trigger Valida_CPF_Lead on Lead (before insert, before update) {

    
        if(trigger.isBefore && (trigger.isInsert || trigger.isUpdate)){
        String debug = 'Valida_CPF_Contato : ';
        
        System.debug('trigger : ' + debug);
        
        for(Lead lead : trigger.new){
            if(lead.CPF__c == null) continue;
            
    
            String CPF_resultado = lead.CPF__c;
            CPF_resultado = CPF_resultado.replace('.', '');
            CPF_resultado = CPF_resultado.replace('-', '');
    
            //valida a quantidade de caracteres
            if(CPF_resultado.length() == 11){
                System.debug(debug + 'cpf: ' + lead.CPF__c);
                ...
                resultado += Integer.valueOf(String.valueOf(lead.CPF__c.substring(0, 1))) * 10;
                ...
                if(CPF_resultado.substring(CPF_resultado.length() - 2, CPF_resultado.length())
                   .equals(lead.CPF__c.substring(lead.CPF__c.length() - 2, lead.CPF__c.length()))){
                    lead.CPF__c = CPF_resultado;
                }else lead.addError('Favor de verificar o CPF é inválido no Lead');
            }
        }
    }
